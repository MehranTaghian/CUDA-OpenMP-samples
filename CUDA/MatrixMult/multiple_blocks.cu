...
__global__ void
matrixMulCUDA(float *C, float *A, float *B, int n, int TILE_WIDTH)
{
...
void fillMat(float * v, int matSizeX, int matSizeY) {
    static int L = 0;
...
*/
int matrixMultiply(int argc, char **argv, int n, int block_size, int grid_size, int tile_width)
{
    ...
    // Execute the kernel
    matrixMulCUDA << < grid, threads >> > (d_C, d_A, d_B, n, tile_width);
...
int main(int argc, char **argv)
{
    ...
    int matrix_result = matrixMultiply(argc, argv, n, block_size, grid_size, tile_width);
