...
void task_part(string output[], int imageWidth, int imageHeight, double minR, double maxR, double minI, double maxI, int maxN, int y);
...
int main()
{
	...
	ofstream fout("output_image_Task.ppm");
...
#pragma omp task
				{
					task_part(output, imageWidth, imageHeight, minR, maxR, minI, maxI, maxN, y);
				}
			}
